...
class Produto_model extends MY_Model {
  ...
  public function listaProduto($id = null){
    ...
    if($id == null){
      ...
      $result = $this->db->query("SELECT id, categoria, codigo, descricao, detalhe, preco_venda, preco_custo, estoque_saldo, estoque_minimo, status, registro, md5(id) as hash FROM produto  WHERE loja_id =  {$this->session->usuario[0]['idloja']};")->result_array();
    }else{
